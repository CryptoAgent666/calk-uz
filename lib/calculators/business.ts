...
export function calculateIPTax(
  ...
  if (regime === 'self_employed') {
    ...
    // НК ст. 467: индивидуальные предприниматели и самозанятые платят налог
    // с оборота по ЕДИНОЙ ставке 1%. 4% — ставка для ЮРИДИЧЕСКИХ лиц
    // (в рознице вне городов от 100 тыс. — 2%, в труднодоступных районах — 1%),
    // она к ИП не применяется. Отдельной полосы «4% при обороте свыше 1 млрд»
    // не существует: сверх порога ИП уходит с налога с оборота на НДС + прибыль.
    taxRate = 0.01
    taxAmount = annualRevenue * taxRate
  } else {
